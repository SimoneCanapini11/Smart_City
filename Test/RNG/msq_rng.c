...
#define SERVERS  8                     /* number of servers              */
#define MAX_Q_E 100000                /* Dimensione massima buffer coda E */

double sum_gen_N = 0.0;
long count_gen_N = 0;
...
double GetService_N(void) {                 
     ...
     if (r <= p1) {
         service_time = Exponential(mu_1); // Fase 1: Richiesta veloce
     } else {
         service_time = Exponential(mu_2); // Fase 2: Richiesta lenta
     }

     // Accumula le statistiche per il test RVG
     sum_gen_N += service_time;
     count_gen_N++;
...
double GetService_T(void) {                 
     /* Telemetria: tempo esponenziale standard */
     SelectStream(2);                            
...
// Variabili di stato per le code e i server
  ...
  long       offloaded_N = 0;
  double     last_arrival_time = START;
  ...
  // --- INIZIO TEST DEL GENERATORE RNG ---
  FILE *rng_file = fopen("rng_trace.csv", "w");
  if (rng_file != NULL) {
      fprintf(rng_file, "U_i,U_i_next\n"); // Intestazione
      
      SelectStream(5); // Usiamo uno stream non utilizzato dalla simulazione
      double u_prev = Random(); 
      
      for (int i = 0; i < 10000; i++) {
          double u_next = Random();
          // Scriviamo la coppia di numeri consecutivi
          fprintf(rng_file, "%f,%f\n", u_prev, u_next);
          u_prev = u_next; // Il numero attuale diventa il precedente per il prossimo ciclo
      }
      fclose(rng_file);
  } else {
      printf("Errore: Impossibile creare rng_trace.csv\n");
  }
  // --- FINE TEST DEL GENERATORE RNG ---
  ...
  printf("\n--- VERIFICA: TEST DEI GENERATORI ---\n");
  if (count_gen_N > 0) {
      double expected_mean = 30.0;
      double sample_mean = sum_gen_N / count_gen_N;
      
      // Calcolo dell'errore relativo in percentuale
      double perc_error = fabs(sample_mean - expected_mean) / expected_mean * 100.0;
      
      printf("Media Teorica (Expected Value) Navigazione :  30.00\n");
      printf("Media Campionaria (Sample Mean) Navigazione: %6.2f (su %ld estrazioni)\n", sample_mean, count_gen_N);
      printf("Errore Relativo Percentuale                : %6.3f%%\n", perc_error);
  }
 
  printf("\n--- VERIFICA: SANITY CHECKS SUI GENERATORI ---\n");

  long total_served = index;
  
  if (total_arrivals > 0) {
      // Calcola l'Average Interarrival Time come t.last / total_arrivals
      double avg_interarrival = last_arrival_time / total_arrivals;
      printf("Average interarrival time : %6.4f ms (Valore Teorico Atteso:  2.0000 ms)\n", avg_interarrival);
  }
  
  if (total_served > 0) {
      // Calcola il tempo di servizio totale sommando quello di ogni server
      double total_service_time = 0.0;
      for (s = 1; s <= SERVERS; s++) {
          total_service_time += sum[s].service;
      }
      
      // Calcola l'Average Service Time come area.service / index
      double avg_service = total_service_time / total_served;
      printf("Average service time      : %6.4f ms (Valore Teorico Atteso: 15.5000 ms)\n", avg_service);
  }
  printf("----------------------------------------------------------------\n");
