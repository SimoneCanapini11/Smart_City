...
// Variabili di stato per le code e i server
  ...
  while ((event[0].x != 0) || (busy_servers > 0) || (queue_E > 0) || (queue_N > 0) || (queue_T > 0)) {
    ...
    //Scrivi lo stato nel CSV
    long total_in_system = queue_E + queue_N + queue_T + busy_servers;
    fprintf(fp_csv, "%.4f,%ld,%d,%ld,%ld,%ld\n", 
            t.current, total_in_system, busy_servers, queue_N, queue_E, queue_T);
  }  
  ...
  printf("\n--- VERIFICA: CONTROLLO DELLA CONSERVAZIONE FLUSSO (FLOW BALANCE) ---\n");
  
  long total_served = index;  
  long jobs_in_system = queue_E + queue_N + queue_T + busy_servers; 
  
  printf("Totale Entità Entrate (Arrivi totali)   : %ld\n", total_arrivals);
  printf("Totale Entità Uscite  (Serviti + Deviati) : %ld (%ld + %ld)\n", 
         total_served + offloaded_N, total_served, offloaded_N);
  printf("Entità rimaste nel sistema (in chiusura) : %ld\n", jobs_in_system);
  
  long check_sum = total_served + offloaded_N + jobs_in_system;
  
  if (total_arrivals == check_sum) {
      printf("Risultato del Test: SUPERATO (Flow Balance perfetto!)\n");
  } else {
      printf("Risultato del Test: FALLITO! (Persi o duplicati %ld job)\n", total_arrivals - check_sum);
  }
