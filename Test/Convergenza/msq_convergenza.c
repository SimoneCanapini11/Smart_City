...
// Variabili di stato per le code e i server
  ...
  // --- APERTURA FILE PER GRAFICO DI CONVERGENZA ---
  FILE *conv_file = fopen("convergence.csv", "w");
  if (conv_file == NULL) {
      fprintf(stderr, "Errore fopen: %s (errno %d)\n", strerror(errno), errno);
      return 1;
  }
  // Intestazione del file
  fprintf(conv_file, "Time,Avg_Queue_E,Avg_Queue_N,Avg_Queue_T\n");
  ...
  while ((event[0].x != 0) || (busy_servers > 0) || (queue_E > 0) || (queue_N > 0) || (queue_T > 0)) {
    ...
    // --- SALVATAGGIO MEDIE CUMULATIVE NEL TEMPO ---
    if (t.current > 0.0) {
        fprintf(conv_file, "%f,%f,%f,%f\n", 
                t.current, 
                area_qE / t.current, 
                area_qN / t.current, 
                area_qT / t.current);
