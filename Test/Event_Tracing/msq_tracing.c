...
// Variabili di stato per le code e i server
  ...
  FILE *fp_csv = fopen("trace.csv", "w");
  if (fp_csv == NULL) {
      fprintf(stderr, "Errore fopen: %s (errno %d)\n", strerror(errno), errno);
      return 1;
  }
  // Stampa l'header 
  fprintf(fp_csv, "Time,Total_In_System,Busy_Servers,Queue_N,Queue_E,Queue_T\n");
  ...
  while ((event[0].x != 0) || (busy_servers > 0) || (queue_E > 0) || (queue_N > 0) || (queue_T > 0)) {
    ...
    if (e == 0) {                                  /* process an arrival*/
      ...
      if (r < 0.10) { 
        ...
        printf("t=%6.2f: [ARRIVO] Emergenza\n", t.current);
      }
      else if (r < 0.40) { 
        job_class = 2; arrivals_N++; 
        printf("t=%6.2f: [ARRIVO] Navigazione\n", t.current); 
      }
      else { 
        job_class = 3; arrivals_T++; 
        printf("t=%6.2f: [ARRIVO] Telemetria\n", t.current); 
      }        
      ...
      if (queue_E > 0) {
        queue_E--;
        printf("t=%6.2f: [SERVIZIO] Iniziato servizio Emergenza (rimasti in coda E: %ld)\n", t.current, queue_E);
        ...
        queue_N--;
        printf("t=%6.2f: [SERVIZIO] Iniziato servizio Navigazione (rimasti in coda N: %ld)\n", t.current, queue_N);
        ...
        queue_T--;
        printf("t=%6.2f: [SERVIZIO] Iniziato servizio Telemetria (rimasti in coda T: %ld)\n", t.current, queue_T);
