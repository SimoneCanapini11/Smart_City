...
#define SERVERS  12                      /* number of servers              */
...
double GetService_T(void) {                 
     ...
     SelectStream(2);   // Stream Telemetria                           
...
// Variabili di stato per le code e i server
  ...
  // Variabili per ADAPTIVE TRUNK RESERVATION
  int        busy_servers_non_E = 0;   // Server occupati da Navigazione o Telemetria
  int        limit_non_E = SERVERS;    // Limite dinamico 
  int        in_crisis_mode = 0;       // Flag di stato (0 = Normale, 1 = Crisi)   
  ...
  double recovery_time_E = -1.0;     /* tempo di ripristino della sola coda Emergenza */
  int    is_recovered_E = 0;

  double recovery_time_full = -1.0;   /* tempo di ripristino di tutte le code (E, N, T) */
  int    is_recovered_full  = 0;

  double baseline_E = 0.0, baseline_N = 0.0, baseline_T = 0.0; /* livello medio pre-incidente */
  ...
  fprintf(f_log, "Time,Queue_E,Queue_N,Queue_T,limit_non_E\n");
  ...
  while ((event[0].x != 0) || (busy_servers > 0) || (queue_E > 0) || (queue_N > 0) || (queue_T > 0)) {
    ...
    if (t.current >= next_log_time) {
        fprintf(f_log, "%f,%ld,%ld,%ld,%d\n", t.current, queue_E, queue_N, queue_T, limit_non_E);
    ...
    // Verifica del Recovery Time per l'intero sistema
    ...
    // --- CALCOLO BASELINE PRE-INCIDENTE (livello medio "normale" delle code) ---
    ...
    if (e == 0) {                                  /* process an arrival*/
      ...
      if (t.current > ACCIDENT_TIME && t.current <= ACCIDENT_END) {
          ...
          prob_N = 0.85; // La Navigazione occupa il 50% del taffico
      }
      ...
      // --- ATTIVAZIONE CRISI (TRUNK RESERVATION) ---
      if (in_crisis_mode == 0 && expected_wait >= 40.0) {
          in_crisis_mode = 1;
          limit_non_E = 9; // Riserva fisicamente c-K server in esclusiva per le Emergenze
      }
      ...
      if (job_class == 2 && expected_wait > 50.0) {
          ...
          if (busy_servers < SERVERS) {
            ...
            // SE IL JOB NON E' UN'EMERGENZA, VERIFICA IL LIMITE HARDWARE
            if (job_class != 1 && busy_servers_non_E >= limit_non_E) {
                // Viene forzato in coda per mantenere il server "Idle" per l'Emergenza
                if (job_class == 2) {
                    queue_N++;
                ...
                if (job_class != 1) busy_servers_non_E++; // Aggiorna contatore Non-Emergenze
      ...
      // --- LOGICA DI RILASCIO GRADUALE (SOFT RECOVERY) ---
      if (server_job_class[s] != 1) {
          busy_servers_non_E--; // Un server occupato dal traffico non critico si è liberato

          double current_expected_wait = ((queue_E * 5.0) + (queue_N * 30.0)) / SERVERS;
          
          // Se siamo in crisi, ma il sistema è tornato stabile 
          if (in_crisis_mode == 1 && current_expected_wait <= 10.0 && queue_E == 0 && queue_N == 0) {
              if (limit_non_E < SERVERS) {
                  limit_non_E++; // Restituisce gradualmente un server alla volta
              }
              if (limit_non_E == SERVERS) {
                  in_crisis_mode = 0; // Crisi ufficialmente rientrata
              }
          }
      }
      ...
      if (queue_E > 0) {
      ...
      else if (queue_N > 0 && busy_servers_non_E < limit_non_E) {
        // PRELEVA N DALLA CODA SOLO SE NON SUPERA IL LIMITE ATTUALE
        queue_N--;
      ...
      else if (queue_T > 0 && busy_servers_non_E < limit_non_E) {
        // PRELEVA T DALLA CODA SOLO SE NON SUPERA IL LIMITE ATTUALE
        queue_T--;
        server_job_class[s] = 3; 
        busy_servers_non_E++;
        ...
        // Se c'è traffico in coda (N o T) ma il limite "non_E" è stato raggiunto, 
        // il server non processa nulla e si mette in Idle per proteggere l'Emergenza.
        busy_servers--;
        event[s].x = 0;
      }
    }
