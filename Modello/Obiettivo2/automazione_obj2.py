...
if len(tassi_offload) > 0:
    ...
    print("2. TENUTA DEL VINCOLO DI EMERGENZA (Durante Incidente):")
...
try:
    ...
    # Plot delle tre code
    plt.plot(tempi, code_E, color='red', label="Emergenza (Alta Prio)", linewidth=2)
    plt.plot(tempi, code_N, color='blue', label="Navigazione (Media Prio)", alpha=0.8)
    plt.plot(tempi, code_T, color='orange', label="Telemetria (Bassa Prio)", alpha=0.8)
    
    # Scala logaritmica sull'asse Y per visualizzare ordini di grandezza diversi
    plt.yscale('symlog')
    
    plt.axvline(x=1000000.0, color='black', linestyle='--', label="Inizio Incidente")
    plt.axvline(x=1500000.0, color='green', linestyle='--', label="Fine Incidente")
    
    plt.title("Evoluzione Transitoria delle Code (Scala Logaritmica)", fontsize=14)
    plt.xlabel("Tempo di Simulazione (ms)", fontsize=12)
    plt.ylabel("Numero di Job in Coda (Log)", fontsize=12)
    plt.grid(True, linestyle=':', alpha=0.7)
    plt.legend()
    
    plt.savefig("grafico_incidente.png", dpi=300, bbox_inches='tight')
    plt.show()
    print("Grafico salvato con successo come 'grafico_incidente.png'.")
