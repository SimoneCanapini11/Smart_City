...
#define STOP         (WARMUP + (NUM_BATCHES * BATCH_SIZE))    /* 25.500.000 ms */
...
{     
  ...
  arrival += Exponential(2.0);  // 500 arrivi al sec
  return (arrival);
}
  ...
  int main(int argc, char *argv[])
{
  ...
  while ((event[0].x != 0) || (busy_servers > 0) || (queue_E > 0) || (queue_N > 0) || (queue_T > 0)) {
    ...
    if (e == 0) {                                  /* process an arrival*/
      ...
      SelectStream(3);  // Stream per determinare classe arrivi
      double r = Random();
      int job_class;
      ...
      if (job_class == 2 && expected_wait > 50.0) {
          offloaded_N++;
          // Il job viene inviato al nodo adiacente, non entra nel sistema
      }  else {
